...
Sky::Sky(ID3D11Device* device, const std::wstring& cubemapFilename)
{
	...
	std::vector<XMFLOAT3> vertices(box.Vertices.size());

	for (size_t i = 0; i < box.Vertices.size(); ++i)
		vertices[i] = box.Vertices[i].Position;
	...
	indices16.assign(box.Indices.begin(), box.Indices.end());
...
void Sky::Draw(ID3D11DeviceContext* dc, XMMATRIX &mProj, XMMATRIX &mView, FLOAT obrx, FLOAT obry, FLOAT Scalef, FLOAT Tx, FLOAT Ty, FLOAT Tz)
{
	XMMATRIX Scale = XMMatrixScaling(Scalef, Scalef, Scalef);
	XMMATRIX Translate = XMMatrixTranslation(Tx, Ty, Tz);
	XMMATRIX RotY = XMMatrixRotationY(obrx);
	XMMATRIX RotX = XMMatrixRotationX(obry);

	XMMATRIX WVP = XMMatrixMultiply(Scale * RotY * Translate * RotX, mView * mProj);
